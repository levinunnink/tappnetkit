...
const TKResourceID ANUnspecifiedPostID = 0;

NSInteger NetworkActivityCount;

@interface TKSession ()

@property (strong,nonatomic) _TKIdentifiedResourceSet * resourceSet;
...
+ (TKSession *)defaultDefaultSession {
    static TKSession * singleton;
    static dispatch_once_t once;
    
    dispatch_once(&once, ^{
        singleton = [[TKSession alloc] init];
    });
    
    return singleton;
}

static TKSession * DefaultSession = nil;
...
- (void)completeUserRequest:(TKUserRequestCompletion)completion withResponse:(TKResponse*)response representation:(NSDictionary*)rep error:(NSError*)error {
    if(rep) {
        TKUser * user = [[TKUser alloc] initWithRepresentation:rep session:self];
...
- (void)completeUserListRequest:(TKUserListRequestCompletion)completion withResponse:(TKResponse*)response representation:(NSArray*)rep error:(NSError*)error {
    if(rep) {
        ...
        for(NSDictionary * userRep in rep) {
            TKUser * user = [[TKUser alloc] initWithRepresentation:userRep session:self];
...
- (void)completeFilterRequest:(TKFilterRequestCompletion)completion withResponse:(TKResponse*)response representation:(NSDictionary*)rep error:(NSError*)error {
    if(rep) {
        TKFilter * filter = [[TKFilter alloc] initWithRepresentation:rep session:self];
...
- (void)completeFilterListRequest:(TKFilterListRequestCompletion)completion withResponse:(TKResponse*)response representation:(NSArray*)rep error:(NSError*)error {
    if(rep) {
        NSMutableArray * filters = [[NSMutableArray alloc] initWithCapacity:rep.count];
        for(NSDictionary * filterRep in rep) {
            TKFilter * filter = [[TKFilter alloc] initWithRepresentation:filterRep session:self];
...
- (void)completeStreamMarkerRequest:(TKStreamMarkerRequestCompletion)completion withResponse:(TKResponse *)response representation:(NSDictionary *)rep error:(NSError *)error {
    if(rep) {
        TKStreamMarker * marker = [[TKStreamMarker alloc] initWithRepresentation:rep session:self];
...
- (id)uniqueResource:(TKIdentifiedResource *)r {
    __block TKIdentifiedResource * resource = r;
    ...
    dispatch_sync(self.resourceUniquingQueue, ^{
        TKIdentifiedResource * existingResource = [self.resourceSet existingResource:resource];
